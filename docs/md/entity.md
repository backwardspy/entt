...
only iterate, read and update specific entities and components.<br/>
It is a subtle difference that can help designing a better software sometimes.

More in details, views are a non-intrusive tool to get access to entities and
...
The view pack allows users to combine multiple views (whether they belong to the
same registry or to different registries) into a single _view-like_ iterable
...
the constness of the registry to their types. As an example:
